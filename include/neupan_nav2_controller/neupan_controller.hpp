...
protected:
   ...
   * @return True if NumPy is compatible with current Python ABI
   */
  bool checkNumpyCompatibility();

  /**
   * @brief Initialize NumPy with fallback strategies
   * @return True if NumPy initialization succeeded
   */
  bool initializeNumpyWithFallback();

  /**
   * @brief Try alternative NumPy import methods
   * @return True if any method succeeded
   */
  bool tryAlternativeNumpyImport();
  ...
  /**
   * @brief Decide whether the new plan is different enough to update NeuPAN
   * @param path Candidate plan
   * @return True if NeuPAN should receive this plan
   */
  bool shouldUploadPlanToNeuPAN(const nav_msgs::msg::Path & path);
  ...
  double self_point_filter_radius_;
  double robot_length_;
  double robot_width_;
  double footprint_filter_padding_;
  double plan_update_min_interval_;
  double plan_start_change_tolerance_;
  double plan_goal_change_tolerance_;

  // Cached signature of the last plan uploaded into NeuPAN
  bool has_uploaded_plan_signature_{false};
  size_t last_uploaded_plan_size_{0};
  double last_uploaded_plan_start_x_{0.0};
  double last_uploaded_plan_start_y_{0.0};
  double last_uploaded_plan_goal_x_{0.0};
  double last_uploaded_plan_goal_y_{0.0};
  std::chrono::steady_clock::time_point last_uploaded_plan_time_{};
